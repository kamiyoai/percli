...
pub fn handler(ctx: Context<Deposit>, account_idx: u16, amount: u128) -> Result<()> {
    ...
    engine
        .deposit(account_idx, amount, oracle_price, clock.slot)
        .map_err(from_risk_error)?;

    // TODO: integrate SPL token transfer from user to vault
