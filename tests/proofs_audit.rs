...
fn proof_epoch_snap_zero_on_position_zeroout() {
    ...
    engine
        .deposit(idx as u16, 1_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_flat_account_maintenance_healthy() {
    ...
    engine
        .deposit(idx, capital as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_flat_zero_equity_not_maintenance_healthy() {
    ...
    assert!(
        !healthy,
        "flat account with zero equity is NOT maintenance-healthy"
    );
}
...
fn proof_fee_debt_sweep_checked_arithmetic() {
    ...
    // Set up capital
    engine
        .deposit(idx as u16, capital as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_keeper_crank_invalid_partial_no_action() {
    ...
    engine
        ...
        .unwrap();
    engine
        .deposit(b, 50_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    ...
    let result =
        engine.keeper_crank_not_atomic(DEFAULT_SLOT + 1, crash_oracle, &candidates, 10, 0i64);
    assert!(
        result.is_ok(),
        "keeper_crank_not_atomic must not revert on invalid partial hint"
    );
...
fn proof_liquidate_missing_account_no_market_mutation() {
    ...
    assert!(
        ...
        "must return Ok(false) for missing account"
    );
...
fn proof_keeper_hint_none_returns_none() {
    ...
    engine
        ...
        .unwrap();
    engine
        .deposit(b, 100_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_withdraw_no_crank_gate() {
    ...
    let result = engine.withdraw_not_atomic(idx, 1_000, DEFAULT_ORACLE, far_slot, 0i64);
    assert!(
        result.is_ok(),
...
/// execute_trade_not_atomic must succeed even when no keeper_crank_not_atomic has ever run.
/// Spec §10.5 does not gate execute_trade_not_atomic on keeper liveness.
#[kani::proof]
#[kani::unwind(34)]
#[kani::solver(cadical)]
fn proof_trade_no_crank_gate() {
    ...
    let result =
        engine.execute_trade_not_atomic(a, b, DEFAULT_ORACLE, far_slot, size, DEFAULT_ORACLE, 0i64);
    assert!(
        result.is_ok(),
        "trade must not require fresh crank (spec §0 goal 6)"
    );
}
...
/// run the actual keeper_crank_not_atomic and verify it succeeds (doesn't fall back
...
fn proof_validate_hint_preflight_conservative() {
    ...
    if let Some(LiquidationPolicy::ExactPartial(q)) = validated {
        ...
        assert!(
            result.is_ok(),
            "keeper_crank_not_atomic must succeed when pre-flight approved ExactPartial"
        );
    ...
    kani::cover!(
        matches!(validated, Some(LiquidationPolicy::FullClose)),
        "pre-flight escalated to full close"
    );
}
...
fn proof_validate_hint_preflight_oracle_shift() {
    ...
    if let Some(LiquidationPolicy::ExactPartial(q)) = validated {
        ...
        let result = engine.keeper_crank_not_atomic(slot2, crank_oracle, &candidates, 10, 0i64);
...
fn proof_force_close_resolved_with_position_conserves() {
    ...
    assert!(
        result.is_ok(),
        "force_close must succeed with open position"
    );
...
fn proof_force_close_resolved_with_profit_conserves() {
    ...
    let idx = engine.add_user(0).unwrap();
    engine
        .deposit(idx, 500_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_force_close_resolved_flat_returns_capital() {
    ...
    engine
        .deposit(idx, dep as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_force_close_resolved_position_conservation() {
    ...
    engine
        .keeper_crank_not_atomic(DEFAULT_SLOT + 1, 1500, &[], 64, 0i64)
        .unwrap();
...
fn proof_maintenance_fee_large_dt_no_revert() {
    ...
    assert!(
        result.is_ok(),
        "large dt must not revert with correct MAX_PROTOCOL_FEE_ABS"
    );
    assert!(engine.check_conservation());
}
