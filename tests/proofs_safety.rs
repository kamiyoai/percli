...
fn bounded_deposit_conservation() {
    ...
    engine
        .deposit(idx, amount as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn bounded_withdraw_conservation() {
    ...
    let idx = engine.add_user(0).unwrap();
    engine
        .deposit(idx, deposit as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    ...
    let result =
        engine.withdraw_not_atomic(idx, amount as u128, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
...
fn bounded_trade_conservation() {
    ...
    engine
        .deposit(a, dep as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, dep as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn bounded_liquidation_conservation() {
    ...
    engine
        .deposit(a, deposit_amt as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn bounded_margin_withdrawal() {
    ...
    let result =
        engine.withdraw_not_atomic(a, withdraw_amt as u128, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
    assert!(result.is_ok());
    assert!(engine.check_conservation());
    ...
    if remaining < u128::MAX {
        let result2 =
            engine.withdraw_not_atomic(a, remaining + 1, DEFAULT_ORACLE, DEFAULT_SLOT, 0i64);
...
fn proof_multiple_deposits_aggregate_correctly() {
    ...
    engine
        .deposit(a, amount_a as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, amount_b as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_close_account_returns_capital() {
    ...
    let idx = engine.add_user(0).unwrap();
    engine
        .deposit(idx, 50_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_trade_pnl_is_zero_sum_algebraic() {
    ...
    engine
        .deposit(a, 5_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, 5_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_protected_principal() {
    ...
    engine
        .deposit(a, dep_a as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, dep_b as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_withdraw_simulation_preserves_residual() {
    ...
    engine
        .execute_trade_not_atomic(a, b, 100, 1, size_q, 100, 0i64)
        .unwrap();
    ...
    assert!(
        result.is_ok(),
        "withdraw_not_atomic of 1000 from 10M capital must succeed"
    );
...
fn proof_funding_rate_validated_before_storage() {
    ...
    let result = engine.keeper_crank_not_atomic(1, 100, &[(a, None)], 1, 0i64);
    kani::cover!(result.is_ok(), "crank Ok path reachable");

    if result.is_ok() {
        let stored = engine.funding_rate_bps_per_slot_last;
        assert!(
            stored.abs() <= MAX_ABS_FUNDING_BPS_PER_SLOT,
    ...
    let result2 = engine.keeper_crank_not_atomic(2, 100, &[(a, None)], 1, 0i64);
    assert!(
        result2.is_ok(),
...
fn proof_min_liq_abs_does_not_block_liquidation() {
    ...
    let b = engine.add_user(0).unwrap();
    engine
        .deposit(a, 50_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
...
fn proof_trading_loss_seniority() {
    ...
    let a = engine.add_user(0).unwrap();
    engine
        .deposit(a, 10_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_risk_reducing_exemption_path() {
    ...
    let reduce_result = engine.execute_trade_not_atomic(
        b,
        a,
        DEFAULT_ORACLE,
        DEFAULT_SLOT,
    ...
    engine2
        .deposit(a2, 100_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine2
        .deposit(b2, 500_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine2
        .execute_trade_not_atomic(
            a2,
    ...
    let increase_result = engine2.execute_trade_not_atomic(
        a2,
        b2,
        DEFAULT_ORACLE,
        DEFAULT_SLOT,
...
fn proof_buffer_masking_blocked() {
    ...
    engine
        .deposit(victim, 100_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(attacker, 500_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_fee_debt_sweep_consumes_released_pnl() {
    ...
    engine
        .deposit(idx, cap as u128, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_touch_drops_excess_at_fee_credits_limit() {
    ...
    assert!(
        engine.accounts[a as usize].fee_credits.get() == -(i128::MAX),
...
fn proof_gc_reclaims_flat_dust_capital() {
    ...
    let idx = engine.add_user(0).unwrap();
    engine
        .deposit(idx, 10_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_property_3_oracle_manipulation_haircut_safety() {
    ...
    engine
        .deposit(a, 500_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, 500_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .keeper_crank_not_atomic(DEFAULT_SLOT, DEFAULT_ORACLE, &[], 0, 0i64)
        .unwrap();
    ...
    let slot2 = DEFAULT_SLOT + 1;
    engine
        .keeper_crank_not_atomic(slot2, spike_oracle, &[(a, None), (b, None)], 64, 0i64)
        .unwrap();
    ...
    assert!(
        pnl_a > 0,
        "account a must have positive PnL after oracle spike"
    );
    ...
    assert!(
        h_den_after <= h_den_before || h_den_before == 0,
    ...
    let slot3 = slot2;
    let withdraw_result = engine.withdraw_not_atomic(a, 500_001, spike_oracle, slot3, 0i64);
    assert!(
...
fn proof_property_26_maintenance_vs_im_dual_equity() {
    ...
    engine
        .deposit(a, 20_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, 1_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .keeper_crank_not_atomic(DEFAULT_SLOT, DEFAULT_ORACLE, &[], 0, 0i64)
        .unwrap();
    ...
    let slot2 = DEFAULT_SLOT + 1;
    engine
        .keeper_crank_not_atomic(slot2, new_oracle, &[(a, None), (b, None)], 64, 0i64)
        .unwrap();
    ...
    let maint_healthy =
        engine.is_above_maintenance_margin(&engine.accounts[a as usize], a as usize, new_oracle);
    assert!(
        maint_healthy,
    ...
    engine
        .keeper_crank_not_atomic(slot3, new_oracle, &[(a, None)], 64, 0i64)
        .unwrap();
    ...
    assert!(
        eq_maint_after_warmup >= eq_maint_before_warmup,
...
fn proof_audit_fee_sweep_pnl_conservation() {
    ...
    engine
        .deposit(a, 100, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_audit_k_pair_chronology_not_inverted() {
    ...
    let slot2 = DEFAULT_SLOT + 1;
    engine
        .keeper_crank_not_atomic(slot2, high_oracle, &[(a, None), (b, None)], 64, 0i64)
        .unwrap();

    // a (long) must gain PnL when oracle rises
    assert!(
        engine.accounts[a as usize].pnl > pnl_a_before,
...
fn proof_audit2_close_account_structural_safety() {
    ...
    let result = engine.close_account_not_atomic(a, DEFAULT_SLOT, DEFAULT_ORACLE, 0i64);
...
fn proof_audit2_funding_rate_clamped() {
    ...
    let slot2 = DEFAULT_SLOT + 1;
    let result =
        engine.keeper_crank_not_atomic(slot2, DEFAULT_ORACLE, &[(a, None), (b, None)], 64, 0i64);
...
fn proof_audit4_materialize_at_freelist_integrity() {
    ...
    engine
        .deposit(2, 500, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_audit5_reclaim_dust_sweep() {
    ...
    assert!(
        engine.insurance_fund.balance.get() == ins_before + 500,
...
fn proof_partial_liquidation_can_succeed() {
    ...
    engine
        .deposit(a, 1_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, 1_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
...
fn proof_sign_flip_trade_conserves() {
    ...
    engine
        .deposit(a, 2_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    engine
        .deposit(b, 2_000_000, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    ...
    let slot2 = DEFAULT_SLOT + 1;
    let result =
        engine.execute_trade_not_atomic(b, a, DEFAULT_ORACLE, slot2, size2, DEFAULT_ORACLE, 0i64);
...
fn proof_close_account_fee_forgiveness_bounded() {
    ...
    let idx = engine.add_user(0).unwrap();
    engine
        .deposit(idx, 1, DEFAULT_ORACLE, DEFAULT_SLOT)
        .unwrap();
    ...
    // close_account_not_atomic should succeed: position=0, pnl=0, capital=1 < min_deposit=2
    let result = engine.close_account_not_atomic(idx, DEFAULT_SLOT, DEFAULT_ORACLE, 0i64);
    assert!(
        result.is_ok(),
        "close_account_not_atomic must succeed for dust account with fee debt"
    );
...
fn proof_convert_released_pnl_conservation() {
    ...
    if released > 0 {
        ...
        let result =
            engine.convert_released_pnl_not_atomic(a, x_req as u128, high_oracle, slot2 + 1, 0i64);
        kani::cover!(
            result.is_ok(),
            "convert_released_pnl_not_atomic Ok path reachable"
        );
        if result.is_ok() {
            assert!(
                engine.check_conservation(),
        ...
        assert!(
            engine.check_conservation(),
            "conservation holds even on err path"
        );
    }
}
...
fn proof_convert_released_pnl_exercises_conversion() {
    ...
    assert!(
        released > 0,
        "released must be > 0 with warmup=0 and positive PnL"
    );

    let cap_before = engine.accounts[a as usize].capital.get();

    // Convert all released profit
    let result = engine.convert_released_pnl_not_atomic(a, released, high_oracle, slot2 + 1, 0i64);
    assert!(
        result.is_ok(),
